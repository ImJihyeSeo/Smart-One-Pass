from PySide6.QtWidgets import (
    QWidget, QLabel, QVBoxLayout, QPushButton, QFrame, QHBoxLayout, QSizePolicy, QMessageBox
)
    ...
    def fetch_seat_info(self):
        ...
        try:
            ...
            if response.status_code == 200:
                ...
                # 시간 계산
                # (실제로는 현재 시간과 비교해서 잔여 시간을 계산해야 하지만, 여기서는 단순 표시용으로 처리)
                start_str = reservation['start_time'][:5] # HH:MM
                end_str = reservation['end_time'][:5]
                
                # 종료 1시간 전 시간 계산 (연장 가능 시간)
                try:
                    end_dt = datetime.strptime(reservation['end_time'], "%H:%M:%S")
                    extend_possible_dt = end_dt - requests.timedelta(hours=1) # timedelta import 필요하면 수정
                    # 간단히 문자열 파싱으로 대체하거나 datetime 사용
                    pass
                except:
                    pass
                ...
                self.seat_data["assigned_time_start"] = start_str
                self.seat_data["assigned_time_end"] = end_str
                
                # 잔여 시간 등은 현재 시간 기준으로 계산 필요 (여기서는 임시 값)
                self.seat_data["remaining_time_min"] = 120 # 예시
                self.seat_data["total_time_min"] = 180 # 3시간
                self.seat_data["extend_time"] = end_str # 일단 종료 시간으로 표시
